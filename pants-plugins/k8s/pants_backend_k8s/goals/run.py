...
from pants.engine.fs import EMPTY_DIGEST, MergeDigests
...
async def prepare_kubernetes_command_process(
    ...
    deps, kubeconfig_target, environment_name = await concurrently(
        resolve_targets(**implicitly(DependenciesRequest(kubernetes_command[KubernetesTemplateDependency]))),
        resolve_target(
            WrappedTargetRequest(
        ...
        resolve_environment_name(EnvironmentNameRequest.from_target(request.target)),
    )

    kubeconfig_request = await kubeconfig_request_request(
        KubeconfigRequestRequest(kubeconfig_target.target), **implicitly()
    )

    sources, kubeconfig, tool = await concurrently(
        determine_source_files(
            SourceFilesRequest(
                sources_fields=[d.get(SourcesField) for d in deps],
                for_sources_types=(KubernetesSourceField,),
        ...
        download_external_tool(tool.get_request(platform)),
    )
...
async def run_kubernetes_command_target(request: RunKubernetesCommand) -> RunRequest:
    ...
    process = await prepare_kubernetes_command_process(
        KubernetesCommandProcessRequest(wrapped_tgt.target), **implicitly()
    )
    return RunRequest(
