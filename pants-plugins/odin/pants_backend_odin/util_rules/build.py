...
async def build_odinfmt(
    ...
    downloaded_odin = await Get(DownloadedExternalTool, ExternalToolRequest, odin.get_request(request.platform))
    ...
    process_result = await Get(
        ...
        Process(
            argv=[
                ...
                f"""
                set -e
                git clone https://github.com/DanielGavin/ols.git /tmp/ols
                cd /tmp/ols
                {downloaded_odin.exe} build tools/odinfmt/main.odin -file -show-timings -collection:src=src -out:odinfmt -o:speed
                cp ./odinfmt ./odinfmt_binary
                """,
            ],
            input_digest=input_digest,
            description="Clone OLS and build odinfmt",
            output_files=("./odinfmt_binary",),
